...
function setupScrollAnimations() {
    const observer = new IntersectionObserver((entries, observer) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                ...
                // For photo-card images
                const img = entry.target.querySelector('img');
                if (img && !img.classList.contains('animated')) {
                    img.classList.add('animated'); // prevent re-animation
                }

                // For message texts inside message-card
                const texts = entry.target.querySelectorAll('.message-text');
    ...
    // Observe all fade-in and photo-card elements
    document.querySelectorAll('.fade-in, .photo-card, .section-title, .message-card')
...
const photobserver = new IntersectionObserver ((entries) => {
    entries. forEach(entry => {
        if (entry.isIntersecting) {
            const img = entry.target.querySelector ('img');
            if (img) {
